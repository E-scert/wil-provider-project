...
import { Card, SectionHeading, EmptyState, Spinner, Button, Badge } from '../components/ui.jsx';
            ...
            <Card key={p.program_id} className="animate-fadeUp flex flex-col justify-between">
              <div>
                <div className="mb-2 flex items-start justify-between gap-2">
                  <h3 className="font-display text-base font-semibold text-white">{p.program_name}</h3>
                  <Badge>{p.slots_open} slot{p.slots_open === 1 ? '' : 's'}</Badge>
                </div>
                <p className="text-xs text-tut-gold">{p.company?.comp_name}</p>
                <p className="mt-2 text-sm text-white/55 line-clamp-3">{p.program_desc}</p>
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-white/40">
                  <span className="rounded-full border border-tut-line px-2 py-0.5">{p.program_field}</span>
                  <span className="rounded-full border border-tut-line px-2 py-0.5">{p.duration_months} months</span>
